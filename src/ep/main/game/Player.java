...
public class Player {
    ...
    private static final int nCards = 2;

    private static final String invalidCard = "Carta não está na mão do jogador";
    /**
    ...
    protected void swapCard(Card oldCard, Card newCard) throws InvalidCardException {
        int position = this.cardException(oldCard);
        this.cards[position] = newCard;
    }

    public int cardException(Card card) {
        for (int i = 0; i < nCards; i++) {
            if (this.cards[i].equals(card)) {
                return i;
            }
        }
        throw new InvalidCardException(invalidCard);
    }
}